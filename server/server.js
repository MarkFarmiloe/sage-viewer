...
const loadIndex = async (dataName, indexParts = [1]) => {
    // first build index key
    let idxKey = dataName.toLowerCase();
    indexParts.forEach(indexPart => {
        idxKey += indexPart < 0 ? indexPart : "+" & indexPart;
    })
    // if in cache, return it.
    let idx = store.dataStore[idxKey];
    if (idx) {
        return [idx, null];
    }
    // otherwise try and load it from file, put it in the cache and return it.
    try {
        const stringData = await fs.readFile(path.join(store.datasetPath, idxKey) + ".json", { encoding: 'utf8' });
        idx = JSON.parse(stringData);
        console.log(data);
        store.dataStore[idxKey] = idx;
        return [idx, null];
    } catch (err) {
        // if the problem isn't a missing index file or the default index is missing, crash out.
        if (err !== ENOENT || (indexParts.length === 1 && indexParts[0] === 1)) {
            return [null, err];
        }
        // otherwise build the new index file, add it to the cache and return it.
        [idx, err] = buildIndex(dataName, indexParts);
        if (idx) {
            store.dataStore[idxKey] = idx;
            return [idx, null];
        }
        // else couldn't build the new index, so crash out.
        return [null, err];
    }
}

const loadData = async (dataName) => {
    let data = store.dataStore[dataName];
    if (data) {
        return [data, null];
    }
    try {
        const stringData = await fs.readFile(path.join(store.datasetPath, dataName) + ".json", { encoding: 'utf8' });
        data = JSON.parse(stringData);
        if (data.length <= MAX_ROWS_TO_CACHE) {
            store.dataStore[dataName] = data;
        }
        return [data, null];
    } catch (err) {
        return [null, err];
...
const buildIndex = async (dataName, indexParts) => {
    const [data, err] = await loadData(dataName);
    if (err) {
        return [null, err];
    }
...
const buildIndexA = (array, cmpfn = (a, b) => a < b ? -1 : 1) => {
    const qsi = index => {
